...
export async function execute(interaction: Interaction<CacheType>) {
	if (!savedGuildData.has(interaction.guildId)) savedGuildData.set(interaction.guildId, createSavedGuildDataEntry());
    if (!interaction.isCommand()) return;
	const command = (<ExtendedClient>interaction.client).commands.get(interaction.commandName);
	if (!command) return;
	try {
